...
//Pesan kosong
if (text.includes('!prank')){
    const gh = text.split("!prank ")[1];
...
conn.sendMessage(id, 'Calma ae',MessageType.text, { quoted: m } );
}
if (text.includes("!nome")){
const teks = text.replace(/!nome /, "")
    let nama = `${teks}`;
...
if (text.includes("!descri")){
const teks = text.replace(/!descri /, "")
    let desk = `${teks}`;
    let idgrup = `${id.split("@s.whatsapp.net")[0]}`;
